...
import com.example.ecommerceappmirnes.KelineApplication
...
import com.example.ecommerceappmirnes.util.validateEmail
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: User, imageUri: Uri?){
        ...
        if(!areInputsValid){
            viewModelScope.launch {
                _user.emit(Resource.Error("Check you inputs"))
            }
            return
        }
        viewModelScope.launch {
    ...
    private fun saveUserInformationWithNewImage(user: User, imageUri: Uri) {
        viewModelScope.launch {
            try {
                val imageBitmap= MediaStore.Images.Media.getBitmap(getApplication<KelineApplication>().contentResolver,imageUri)
                val byteArrayDutputStream = ByteArrayOutputStream()
                imageBitmap.compress(Bitmap.CompressFormat.JPEG,96,byteArrayDutputStream)
                val imageByteArray = byteArrayDutputStream.toByteArray()
